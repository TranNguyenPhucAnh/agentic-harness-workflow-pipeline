...
│       └── <version>.changelog.md     ← write-once per-version changelog (spectracker)
│
├── reports/
│   ├── reporter_execution_summary.md
│   └── judge_verdict_summary.md
│
├── src/                               ← build output (executor writes)
└── tests/                             ← build output (scaffolder writes)
```

> `src/` and `tests/` are **build outputs**, not pipeline artifacts.
...
|---|---|---|---|
| `specwright_spec_<slug>.md` | `specwright` | overwrite | spectracker, scaffolder, planner, executor, judge, harness |
| `state/clarificator_requirement_synthesis.md` | `clarificator` | overwrite | enricher, specwright |
| `state/scaffolder_codebase_skeleton.json` | `scaffolder` | overwrite | planner, executor, reporter, judge, harness |
| `state/planner_full_execution_plan.json` | `planner` | overwrite | executor, debugger, reporter, judge, patcher, harness |
| `state/planner_mini_execution_plan.json` | `planner` | overwrite | executor, debugger, reporter, judge, patcher, harness |
| `state/planner_mini_impact_analysis.json` | `planner` | overwrite | executor |
| `state/spectracker_applied_version.json` | `spectracker` | hybrid† | spectracker (self), harness |
| `cache/scaffolder_compressed_spec.md` | `scaffolder` | overwrite | planner, executor, patcher |
| `cache/spectracker_session_version_delta.json` | `spectracker` | overwrite | harness |
| `cache/absorber_session_codebase_snapshot.json` | `absorber` | overwrite | clarificator, enricher, planner |
| `cache/absorber_session_git_snapshot.json` | `absorber` | overwrite | clarificator, enricher |
| `execution/clarificator_session_raw.json` | `clarificator` | overwrite | enricher, planner |
| `execution/clarificator_session_questions.md` | `clarificator` | overwrite | human review |
| `execution/enricher_session_enriched_prompt.md` | `enricher` | overwrite | specwright |
| `execution/executor_session_manifest.json` | `executor` | overwrite | reporter, judge, patcher, harness |
| `execution/debugger_session_test_summary.json` | `debugger` | overwrite | reporter, judge, archivist |
| `execution/judge_session_verdict_raw.json` | `judge` | overwrite | patcher, archivist, harness |
| `execution/patcher_session_fix_summary.md` | `patcher` | overwrite | human review |
| `knowledge/current/clarificator_decision_log.md` | `clarificator` | append | clarificator (next session), enricher |
| `knowledge/current/absorber_codebase_map.md` | `absorber` | overwrite | clarificator, enricher, planner, executor |
| `knowledge/current/absorber_config_map.json` | `absorber` | overwrite | clarificator, enricher |
| `knowledge/current/absorber_blame_map.md` | `absorber` | overwrite | clarificator, enricher, planner |
| `knowledge/current/patcher_findings_snapshot.md` | `patcher` | overwrite | debugger, archivist |
| `knowledge/current/archivist_spec_gaps.md` | `archivist` | append/curated | specwright |
| `knowledge/current/archivist_knowledge_log.md` | `archivist` | append | planner, executor, debugger, patcher, judge |
| `knowledge/history/archivist_curation_log.json` | `archivist` | append | human review |
| `knowledge/history/patcher_attempt_log.json` | `patcher` | append | human review, archivist |
| `knowledge/history/spectracker_version_log.md` | `spectracker` | append | human review |
| `knowledge/history/<version>.md` | `spectracker` | write-once | spectracker (delta computation) |
| `knowledge/history/<version>.changelog.md` | `spectracker` | write-once | human review |
| `reports/reporter_execution_summary.md` | `reporter` | overwrite | human review only |
| `reports/judge_verdict_summary.md` | `judge` | overwrite | human review only |
...
### `knowledge/history/<version>.md` and `<version>.changelog.md`
Write-once files created by spectracker each time a new spec version is applied. `<version>.md` is the raw spec snapshot used internally by spectracker for future delta computation (`_load_latest_snapshot`). `<version>.changelog.md` is the human-readable per-version changelog. Both are dynamic paths constructed at runtime — not static constants in paths.py.
...
| `state/enriched_prompt.md` | Moved to `execution/enricher_session_enriched_prompt.md`. Session artifact; wrong location in state/. |
...
| `knowledge/history/git_history.json` | Point-in-time snapshot semantics, not a persistent log. Moved to `cache/absorber_session_git_snapshot.json`. |
...
2. **cache/** — heavy intermediates regenerable from source. Overwrite each run. Document if used for control flow (exception).
3. **execution/** — outputs of a single pipeline run. All session artifacts (overwrite). Consumed by downstream steps within same run.
4. **knowledge/current/** — files read by pipeline steps across runs. Append-only or curated overwrite.
5. **knowledge/history/** — append-only logs and write-once snapshots. Human audit only unless noted.
6. **reports/** — human readability only. Never consumed by pipeline logic.
7. **Build outputs** (`src/`, `tests/`) — written only by scaffolder and executor.
...
- `artifacts/paths.py` — with owner, consumers, lifecycle, purpose comments
- `artifacts/OWNERSHIP.md` — in ownership table
